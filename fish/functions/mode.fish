	...
	if [ $arg_count -eq 1 ];
		...
		if test "$mode_command" = "android"
			if test "$ENABLED_ANDROID" = "false"
				...
				defaults write com.pewpewthespells.fish.modes ENABLE_ANDROID true
				...
				defaults write com.pewpewthespells.fish.modes ENABLE_ANDROID false
		...
		if test "$mode_command" = "work"
			...
			if test "$ENABLED_WORK" = "false"
				...
				defaults write com.pewpewthespells.fish.modes ENABLED_WORK false
				sudo chmod -rx /Library/Java/JavaVirtualMachines/jdk*.jdk
			end
