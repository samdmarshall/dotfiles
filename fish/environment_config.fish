...
if [ $FISH_PLATFORM_NAME = "Darwin" ]
    ...
    if not set -q HAS_XCRUNNER
        set env_cmd_test_result false
        if command -s xcrunner >/dev/null
            set env_cmd_test_result true
        end
    ...
    if not set -q HAS_DEFAULTS
        set env_cmd_test_result false
        if command -s defaults >/dev/null
            set env_cmd_test_result true
        end
...
if not set -q HAS_SHUTDOWN
    set env_cmd_test_result false
    if command -s shutdown >/dev/null
        set env_cmd_test_result true
    end
...
if not set -q HAS_PYGMENTIZE
    ...
    if command -s pygmentize >/dev/null
        set env_cmd_test_result true
    end
